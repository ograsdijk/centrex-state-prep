...
| Is the reported `1.7e-04` spread resolved at `10_000`? | **Yes, with room to spare.** Ladder to `N=160000`: the AP2 spread changes by `9.82e-06, 1.31e-07, 3.36e-08, 1.60e-08` over `10k..160k`. Summing the tail puts `N=10_000` within `~1.0e-05` of the limit -- about `6%` of the `1.7e-04` being reported. AP1 is within `~9.2e-05`. The `6.996e-04` in the notebook's own check was `manifold` at the strongest drive, a looser observable than the `matched` number the report quotes | cross-scheme |
| How is the limit known to *be* the limit? | Both ladders bottom out on a precision floor rather than merely slowing: AP2's lineshape residual goes `9.24e-06, 3.05e-08, 7.88e-09, 7.84e-09` -- the last two are indistinguishable -- and AP1 flattens near `4e-08`. A scheme still converging does not stop; one that has hit arithmetic precision does | exact |
...
| Does grading endanger Magnus here? | **No** -- unlike SPA2 it *lowers* `max||A||` rather than raising it (`0.6116 -> 0.4552` at `N=2500`). But the step count does: at `N=2500` the uniform grid already sits at `0.6116`, past the Taylor guard | geometry |
| Was the first `||A||` measurement right? | **No.** `magnus_step_norms` defaults to `coupling_scale=1.0`, the *nominal* field power, and this example runs at prefactors `16.156` / `8.254`. Coupling goes as `sqrt(intensity)`, so the real norms are `4.02x` / `2.87x` larger. The nominal reading said the guard had margin at every step count; it does not at `N=2500`. Pass a scaled field per prefactor -- one `coupling_scale` cannot carry two | geometry |
| Is Magnus's error a detuning shift or a shape change? | **Mostly shape.** At `81` detuning points, removing the best-fit shift takes the RMS gap from `5.44e-03 -> 4.03e-03` (`10k`), `2.26e-03 -> 1.81e-03` (`20k`), `6.26e-04 -> 5.00e-04` (`40k`) -- the shift accounts for only `20-26%`. The shift is real (`0.14-0.94 kHz`) but is the minority of the error | cross-scheme |
| Watch the statistic | An earlier reading of this put the shift at `~80%` by dividing a **max-abs** raw error by an **RMS** aligned residual. Compare like with like: RMS against RMS, or max against max. The same alignment removes `45%` of the max and `20%` of the RMS | theory |
| How is the frozen answer known to be right? | **It is not, in the `exact` sense -- there is no closed form for this setup.** What is measured: the two schemes converge to a *common* limit. Aligning Magnus onto frozen at matched `N` closes as `-3.380, -1.780, -0.360 kHz` with residual `7.6e-03, 2.8e-03, 5.0e-04`, so Magnus is approaching where frozen already sits. Frozen is then converged to the answer two independent discretisations agree on | cross-scheme |
| What would that comparison miss? | Anything the two schemes **share**: the `H_slow` eigensolve, the rotating frame, `reorder_evecs`, the field construction. A defect in any of those moves both identically and cancels. Closing that needs a closed form, which this setup does not admit; the nearest independent check is `bench_analytic`, which drives the same shipped loops against closed forms and finds the frozen path second order -- machinery, not this example's physics inputs | theory |
| Can grading be rescued by grading *harder*? | **No.** `1.581x` is not what grading achieved, it is the best any placement can achieve on this profile: the ceiling comes from equidistribution being optimal, so deviating further *reduces* the placement gain while destroying more telescoping. Within the `p` family more aggressive is better (`p=1` gives `1.581x`, `p=2` gives `1.391x`), but that is picking the top of a curve, not passing it | theory |
| Is there a construction that would work? | **Piecewise-uniform**, not smoother grading. Telescoping survives *within* a block of constant `dt`, so a coarse block plus a fine block across the feature pays a boundary term once per transition instead of once per step. It cannot win here -- the `1.581x` ceiling still caps it against a `5.7x`+ cancellation advantage -- but it is the right construction wherever `1/f` is genuinely large. Check `grading_ceiling` first; it needs no propagation | theory |
...
| Why is Magnus first order here? | **Resolved by computing the dropped term** (`benchmarks/bench_magnus_commutator.py`, no propagation). Magnus-1 truncates `Omega2 = -1/2 int int [H_I(t1), H_I(t2)]`, which is its entire extra error over the frozen propagator. `||Omega2||` falls only *linearly* in `dt` -- the oscillatory kernels saturate at `~1/delta` instead of growing with `dt` -- so `N*||Omega2||` is constant at `1.85e-02` and the dropped term stays a fixed `~0.3%` of the retained one however far the grid is refined. That bound brackets the measured `1.0e-02` at `N=10000` from above, the gap being per-step cancellation | exact |
| Magnus speed against Magnus accuracy | It *is* faster per run, `1.32-1.42x` across `N = 20000..160000`. It loses anyway: `frozen` at `N=40000` (`74 s`) is `4500x` more accurate than `magnus` at `N=160000` (`209 s`). To reach a residual of `1e-05`, `frozen` needs `36 s` and Magnus about `840 s` -- **`~23x` in wall clock**, in the opposite direction to the per-step speed | timing + cross-scheme |
